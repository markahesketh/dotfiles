...
**When creating commits, always use the `commit-creator` skill.** Don't run `git commit` directly with an ad-hoc message. Invoke the `commit-creator` skill so the worktree is inspected, unrelated work is split into atomic commits, and messages follow the project's conventional-commit style. This applies to every commit made during babysitting — review fix-ups, CI fixes, rebase resolutions, anything.
